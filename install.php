...
function install_main() {
  ...
  if (!$task) {
    ...
    // Проверка необходимых параметров. Длится по 4 минуты. Отключаю.
...
function install_settings_form(&$form_state, $profile, $install_locale, $settings_file, $db_url, $db_type, $db_prefix, $db_user, $db_pass, $db_host, $db_port, $db_path) {
  ...
  if (count($db_types) == 0) {
    ...
    // Table prefix
    $prefix = ($profile == 'default') ? 'drushop_' : $profile .'_';
    $form['advanced_options']['db_prefix'] = array(
      '#type' => 'hidden',
      '#title' => st('Table prefix'),
      '#default_value' => 'drushop_',
      '#size' => 45,
  ...
  if (count($locales) == 1) {
    if ($profilename == 'default') {
      ...
      if (!empty($_GET['localize'])) {
        ...
        $output = '<ul><li><a href="install.php?profile='. $profilename .'&amp;locale=en">'. st('Install Drushop in English') .'</a></li><li><a href="install.php?profile='. $profilename .'&amp;localize=true">'. st('Learn how to Install Drushop in other languages') .'</a></li></ul>';
      }
      print theme('install_page', $output);
      exit;
    }
...
More details about installing Drushop are available in INSTALL.txt.', array('@drupal' => drupal_install_profile_name(), '%file' => $file, '%default_file' => $conf_path .'/default.settings.php', '@handbook_url' => 'http://drupal.org/server-permissions')), 'error');
    }
    elseif (!$writable) {
