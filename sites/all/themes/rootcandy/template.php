...
function _rootcandy_admin_navigation() {
  ...
  elseif ($rootcandy_navigation == '_rootcandy_default_navigation') {
    // build default menu
    $menu_tree[] = array('href' => 'admin', 'title' => t('All'));
    ...
    $menu_tree[] = array('href' => 'admin/build', 'title' => t('Site building'));
...
function rootcandy_admin_block_content($content) {
  ...
  if (system_admin_compact_mode()) {
	...
	if ($image = _rootcandy_menu_icon($item['href'])) {
        ...
        $image = theme('image', drupal_get_path('theme', 'rootcandy') .'/icons/i32/misc/unknown.png');
      }
	  $output .= '<li class="admin-row">';
	  $output .= '<div class="admin-img">'. $image .'</div>';
      $output .= '<div class="admin-block">'. '<div class="admin-title">'. l($item['title'], $item['href'], $item['localized_options']) .'</div>';
      $output .= '<div class="description">'. $item['description'] .'</div>' . '</div>';
	  $output .= '</li>';
    }
    $output .= '</ul>';
  }
